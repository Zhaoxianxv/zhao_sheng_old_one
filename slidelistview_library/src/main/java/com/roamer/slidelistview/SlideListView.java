...
public class SlideListView extends ListView {
	...
	private void init(AttributeSet attrs) {
		if (attrs != null) {
			TypedArray styled = getContext().obtainStyledAttributes(attrs,
					R.styleable.SlideListView);
			mAnimationTime = styled.getInteger(
					R.styleable.SlideListView_slideAnimationTime, 0);
			mSlideMode = SlideMode.mapIntToValue(styled.getInteger(
					R.styleable.SlideListView_slideMode, 0));
			mSlideLeftAction = SlideAction.mapIntToValue(styled.getInteger(
					R.styleable.SlideListView_slideLeftAction, 0));
			mSlideRightAction = SlideAction.mapIntToValue(styled.getInteger(
					R.styleable.SlideListView_slideRightAction, 0));
			styled.recycle();
		}
	...
	public int getOpenedPosition(){
		if (mTouchListener!=null) {
			return mTouchListener.getOpendPosition();
		}
	...
	public void onOpend(int position, boolean left) {
		if (DEUBG) {
			Log.d(TAG, (left ? "left" : "right") + " back view "
					+ "is opend at position " + position);
		}
	...
	void onClosed(int position, boolean left) {
		if (DEUBG) {
			Log.d(TAG, (left ? "left" : "right") + " back view "
					+ "is closed at position " + position);
		}
		if (mSlideItemListener != null) {
		...
		if (adapter != null) {
			adapter.registerDataSetObserver(new DataSetObserver() {
				...
				public void onChanged() {
					...
					if (mTouchListener.isOpend()) {
						int position = mTouchListener.getOpendPosition();
						if (position < adapter.getCount()) {
							View view = mTouchListener.getSlideItem()
									.getWrapView();
							adapter.getView(mTouchListener.getOpendPosition(),
									view, SlideListView.this);
						}
						mTouchListener.closeOpenedItem();
					}
					mTouchListener.reset();
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		if (isEnabled() && isSlideEnable()) {
			...
			if (action == MotionEvent.ACTION_DOWN) {
				int downPosition = pointToPosition((int) ev.getX(),
						(int) ev.getY());
