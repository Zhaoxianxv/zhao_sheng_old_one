package com.gy.aac.impl;

import java.sql.Timestamp;

public class TimeUtils {

	/**
	 * 把毫秒，转换3:10
	 * 
	 * @param millisencond
	 * @return
	 */
	public static String convertMilliSecondToMinute2(int millisencond) {
		int oneMinute = 1000 * 60;
		int minutes = millisencond / oneMinute;
		int sencond = (millisencond - minutes * oneMinute) / 1000;
		return getNum(minutes) + ":" + getNum(sencond);
	}

	public static String convertMilliSecondToMinute(long millisencond) {
		int oneMinute = 1000 * 60;
		int minutes = (int) millisencond / oneMinute;
		int sencond = ((int) millisencond - minutes * oneMinute) / 1000;
		int msec = (((int) millisencond) % 1000) / 10;
		return getNum(minutes) + ":" + getNum(sencond) + "." + getNum(msec);
	}

	/**
	 * 把秒，转换成 ：
	 * 
	 * @param millisencond
	 * @return
	 */
	public static String convertSecondToMinute2(int seconds) {
		int oneMinute = 60;
		int minutes = seconds / oneMinute;
		int mSecond = seconds - minutes * oneMinute;
		return getNum(minutes) + ":" + getNum(mSecond);
	}

	public static String getNum(int num) {
		if (num >= 10) {
			return "" + num;
		} else {
			return "0" + num;
		}
	}

	public static String getTimestamp() {
		Timestamp ts = new Timestamp(System.currentTimeMillis());
		return String.valueOf(ts.getTime());
	}
}
